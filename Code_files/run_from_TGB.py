...
for dataset_name in DATASETS:
    ...
    if not dataset_tgb_dir.exists():
        print(f"  [SKIP] No TGB variables found at {dataset_tgb_dir}")
        continue

    # Discover all param-tag subdirectories (e.g. nest40_depth1, nest100_depth2, ...)
    param_dirs = sorted([p for p in dataset_tgb_dir.iterdir() if p.is_dir()])
    if not param_dirs:
        print(f"  [SKIP] No param subdirectories found under {dataset_tgb_dir}")
        continue

    for tgb_dir in param_dirs:
        param_tag = tgb_dir.name
        print(f"\n  --- Param tag: {param_tag} ---")

        # Load pre-computed TGB outputs for this param combination
