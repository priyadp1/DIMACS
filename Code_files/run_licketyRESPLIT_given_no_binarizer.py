...
for dataset_name, cfg in DATASETS.items():
    ...
    for depth_budget, lambda_reg, rashomon_mult in itertools.product(DEPTH_BUDGETS, LAMBDA_REGS, RASHOMON_MULTS):
        ...
        results_dir = BASEDIR / "LicketyRESPLIT_EXP_NoBinarizer" / dataset_name / param_str
        ...
        for ti in range(n_trees):
            votes += model.get_predictions(ti, X_test)
        ...
        with open(results_dir / "licketyresplit_nobinarizer_tree_size.json", "w") as f:
            json.dump(tree_size, f)

        with open(results_dir / "licketyresplit_nobinarizer_results.txt", "w") as f:
            f.write(f"\nAccuracy: {acc}")
