...
print("\nBefore Undersampling, class distribution in training set:\n", y_train.value_counts())

undersampling = RandomUnderSampler(random_state=42)
X_train_resampled, y_train_resampled = undersampling.fit_resample(X_train, y_train)

print("\nAfter Undersampling, class distribution in training set:\n", pd.Series(y_train_resampled).value_counts())
...
OUTPATH = BASEDIR / "datasets" / "Mine" / "diabetes_undersampled.csv"
df_resampled.to_csv(OUTPATH, index=False)
print(f"\nSaved Undersampling-resampled training data to {OUTPATH}")
...
TESTPATH = BASEDIR / "datasets" / "Mine" / "diabetes_undersampled_test.csv"
