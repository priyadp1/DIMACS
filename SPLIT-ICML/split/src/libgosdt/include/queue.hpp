...
class PriorityKeyComparator {
   public:
    // Note that the tbb::concurrent_priority_queue is implemented to pop the
    // item with the highest priority value
    bool operator()(message_type const *left, message_type const *right) { return (*left) < (*right); }
};

struct MembershipKeyHashCompare {
    static size_t hash(message_type *message) { return message->hash(); }
...
typedef tbb::concurrent_priority_queue<message_type *, PriorityKeyComparator, tbb::scalable_allocator<message_type *>>
    queue_type;

typedef tbb::concurrent_hash_map<message_type *, bool, MembershipKeyHashCompare,
                                 tbb::scalable_allocator<std::pair<message_type *const, bool>>>
    membership_table_type;  // FIREWOLF: Fix the static assertion error
...
class Queue {
   public:
    ...
    // @returns true if the message was successfully enqueued and not rejected
    ...
    // @requires message: the 4th item of message must contain an instance of
    // identifier_type with the correct amount of pre-allocated memory to copy
    // assign into
    ...
    // @modifes message: message will be overwritten with a copy the content of
