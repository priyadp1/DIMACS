...
class GOSDTClassifier(ClassifierMixin, BaseEstimator):
    ...
    cart_lookahead_depth: int | None, default=None
        Indicates whether to fill leaves with cart trees instead of leaves after the lookahead depth.
    ...
    def __init__(
            ...
            model_limit: int = 1,
            worker_limit: int = 1,
            cart_lookahead_depth: Optional[int] = None,
            verbose: bool = False,
            debug: bool = False,
    ):
    ...
    def fit(self, X, y, y_ref=None, input_features=None, cost_matrix=None, feature_map=None):
        ...
        # Call the split C++ logic
        self.result_ = gosdt_fit(dataset)

        # Check the split result status and report errors as needed.
    ...
    def __create_configuration(self):
        ...
        if self.depth_budget is not None:
            self.config_.depth_budget = self.depth_budget
        ...
        if self.cart_lookahead_depth is not None:
            self.config_.cart_lookahead_depth = self.cart_lookahead_depth
