...
for i in datasets:
    ...
    emb_no = pacmap.PaCMAP(n_components=2, n_neighbors=6, MN_ratio=0.5, FP_ratio=2.0)
    ...
    for j in parameters:
        ...
        X_tgb_combined = np.vstack([df_tgb_X_train, df_tgb_X_test])
        y_tgb_combined = np.concatenate([df_tgb_y_train, df_tgb_y_test])

        if X_tgb_combined.shape[1] < 2:
            print(f"Skipping {i}/{j}: only {X_tgb_combined.shape[1]} feature(s), need ≥2 for PaCMAP")
            continue

        emb_tgb = pacmap.PaCMAP(n_components=2, n_neighbors=6, MN_ratio=0.5, FP_ratio=2.0,
                                 distance="hamming", knn_backend="annoy")
