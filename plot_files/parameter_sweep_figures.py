"""
Comparison of No Binarizer vs ThresholdBinarizer across parameter settings,
plotted per dataset with param_label on the x-axis.
GOSDT and XGBoost are shown as horizontal reference lines where available.
"""
import re
import json
...
for metric_col, metric_label in METRICS:
    ...
    for ax, dataset in zip(axes, datasets):
        ...
        # Add GOSDT and XGBoost as horizontal reference lines where applicable
        if metric_col in BASELINE_METRICS:
            gosdt_val = gosdt_results.get(dataset, {}).get(metric_col)
            xgb_val   = xgb_results.get(dataset, {}).get(metric_col)
            if gosdt_val is not None:
                ax.axhline(gosdt_val, color=COLORS["GOSDT"], linestyle="--",
                           linewidth=1.5, label=f"GOSDT ({gosdt_val:.3g})")
            if xgb_val is not None:
                ax.axhline(xgb_val, color=COLORS["XGBoost"], linestyle=":",
                           linewidth=1.5, label=f"XGBoost ({xgb_val:.3g})")
    ...
    fig.suptitle(f"{metric_label}: Parameter Sweep (No Binarizer vs ThresholdBinarizer)", fontsize=13, fontweight="bold")
    plt.tight_layout()
    out_path = PLOTS_DIR / f"sweep_{metric_col}.png"
